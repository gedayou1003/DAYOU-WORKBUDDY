...
> **口径拍板（2026-09-24，用户确认）**：因子 2 的对比窗口**确定为 5 日**（含当日）。
> 9/23-noon 与 9/24-morning 两档连续暴露该口径的二义性 —— 同一串量能按 5 日判
> 「未创新低」、按 6 日判「创新低」，**因子 2 得分相反**。定 5 日的理由：
> ① 与因子 3「15F 带宽」的短端属性对齐（都刻画最近数日的收口/缩量状态）；
> ② `vol_hist[-5:]` 含**当日**，即「近 5 个交易日（含当日）新低」，与规则 v5
> 「缩量至**近期**极值」表述一致；③ 6 日会把窗口拉到上周，在节前缩量场景下
> 更容易把「常态回落」误判成「前兆」。
> **该窗口已集中为常量 `VOL_NEWLOW_DAYS`（改参数不用碰逻辑）。**

## 实装状态（2026-09-24 用户拍板：**正式实装**）

变盘倾向分**自 2026-09-24 晨报起为正式流程**，不再是「试算 + 对照」：

1. 脚本 `--payload` 算分 → 结果写入 payload 的 `prob_note` 与报告「剧本概率」附注；
2. **9/24 晨报的映射结果 A35／B37／C28 即为正式值**；
3. v5 现行的 `A40／B35／C25`（未含变盘倾向分）**退为历史对照**，仅在附注中一行提及，
   不再作为并列的「两个版本」。
...
# 量能前兆的对比窗口（**用户 2026-09-24 拍板：取 5 日**）
# 背景：9/23-noon 与 9/24-morning 两档连续暴露口径二义 —— 同样一串量能，
#      按 5 日窗口判「未创新低」，按 6 日窗口判「创新低」，因子 2 得分相反。
# 定 5 日的理由：与因子 3「15F 带宽」的短端属性对齐（都刻画最近数日的收口/缩量状态），
#      且 `vol_hist[-5:]` 含**当日**，即「近 5 个交易日（含当日）新低」，
#      与规则 v5 表述「缩量至近期极值」的「近期」一致；6 日会把窗口拉到上周，
#      在节前缩量场景下更容易把「常态回落」误判成「前兆」。
VOL_NEWLOW_DAYS = 5
...
def factor_volume(evidence, vol_hist):
    """因子 2：量能前兆 —— 缩量至 VOL_NEWLOW_DAYS 日新低，或放量站上关键位。

    vol_hist：近若干日成交量序列（含当日，按时间升序）。给不出就计 0 并标注。

    窗口 =VOL_NEWLOW_DAYS（**用户 2026-09-24 拍板：5 日**）。
    改前该窗口是硬编码 `[-5:]`，无常量、无说明，导致 9/23-noon 与 9/24-morning
    两档反复出现「5 日 vs 6 日」口径争议 —— 现集中到常量并写明理由（见文件头）。
    """
    if not vol_hist or len(vol_hist) < 2:
        return 0, '量能序列缺失或不足（需 ≥2 个；建议给近 %d 日含当日）→ 本因子计 0（不猜）' % (
            VOL_NEWLOW_DAYS,), False
    cur = vol_hist[-1]
    window = vol_hist[-VOL_NEWLOW_DAYS:] if len(vol_hist) >= VOL_NEWLOW_DAYS else vol_hist

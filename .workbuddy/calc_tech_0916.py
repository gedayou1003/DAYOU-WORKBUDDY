# -*- coding: utf-8 -*-
"""9/16 晨报技术指标：日线/60F/15F 的 MACD(12,26,9) + MA55/MA20（数据截至 9/15 收盘）"""
...
out = {}
out['day'] = report('日线', dkline(160))
out['m60'] = report('60F', mkline('m60', 400))
out['m15'] = report('15F', mkline('m15', 600))

day = dkline(160)
out['recent'] = [{'date': r['t'], 'o': r['o'], 'h': r['h'], 'l': r['l'], 'c': r['c']} for r in day[-8:]]
# 近 10 日最低（前低，不含当日）
prev10 = day[-11:-1]
out['prev10_low'] = min(r['l'] for r in prev10)
out['prev10_low_date'] = min(prev10, key=lambda r: r['l'])['t']

with open(os.path.join(BASE, '_tech_0916.json'), 'w', encoding='utf-8') as f:

...
> 技能目录：`~/.workbuddy/skills/chan-signal__skillhub/` —— 上表带 `skills/` 前缀的两个引擎脚本**不在本仓库**（用户级技能目录，本机独有、不随项目仓库同步），故《脚本地图.md》不收录它们。
...
> ⚠️ **本节是 2026-08-21 的历史记录**，保留以存查。其中引擎A（`run_000001_multi.py`）已于 2026-09-08 废弃删除，
> 涉及它的条目（下第 1 条）随之作废；其余各条仍对现行链路有效。现行脚本清单见《脚本地图.md》。

本轮覆盖所有定时任务（晨报/午间/晚间/收盘后）调用的脚本，发现并修复 4 个问题：

1. **【严重】~~`run_000001_multi.py` 日线/周线硬编码日期 `'2026-08-14'`~~**：多维引擎（引擎A）日线/周线取数 end 日期写死为 workspace 创建日，导致日线/周线级别用**过期数据**（比当天少 N 个交易日）。已改为 `datetime.now().strftime('%Y-%m-%d')`。影响：此前晨报/晚间快报里引擎A的日线/周线信号是过期的。修复后日线数据 400→401 根，最新到当天，日线趋势与引擎B一致。**⚠️ 该引擎已于 2026-09-08 废弃删除（方向信号无效），本条修复随之作废，仅存史。**
...
>
> **2026-09-21 更新**：本节表格里的 `skills/chan-signal__skillhub/` 两个引擎脚本**确实存在于技能目录**
> （不在本仓库）；裸写文件名的引用已统一改为带路径前缀，并在 §一 末尾声明技能目录位置，
> 以免读者按 `README` 去 `.workbuddy/` 里找不到。文档腐烂的判定口径见《脚本地图.md·§〇·补7》。

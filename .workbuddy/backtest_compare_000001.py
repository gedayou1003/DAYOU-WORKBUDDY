...
# -*- coding: utf-8 -*-
"""
chan-signal 打分回测 · 多配置对比（confidence 过滤提纯）

阶段1：逐日跑引擎，缓存每日信号快照（省去重复跑引擎）
阶段2：用不同 confidence 阈值重放仓位，对比收益/回撤/调仓

打分（信号主导）：卖点(一卖/二卖=1分、三卖=2分)清仓；买点(一买=8、二买=9、三买=7)+趋势/中枢±0.5；无主导信号=保持；仓位=分数/10
"""
...
CACHE = os.path.join(HERE, '000001_信号流缓存.json')
OUT_HTML = os.path.join(HERE, '..', 'outputs', '000001_chan-signal_回测对比_2026-08-14.html')
OUT_JSON = os.path.join(HERE, '..', 'outputs', '000001_chan-signal_回测对比数据_2026-08-14.json')
...
def render_compare(dates, closes, bench_curve, results, st_list):
    n = len(closes)
    step = max(1, n // 400)
...
<title>000001 chan-signal 回测对比</title>
<style>
...
<h1>上证综指 · chan-signal 回测对比（confidence 过滤提纯）</h1>
...
def main():
    ...
    # 取数重建 closes（snaps 缓存不含 close，需重新取数）
    rows = fetch_day('2022-01-01', '2026-08-14', 1500)
    df = to_df(rows)
    ...
    # 保存对比数据
    out_data = {'dates': dates, 'configs': CONFIGS, 'stats': st_list,
    ...
    html = render_compare(dates, closes, bench_curve, results, st_list)

...
> ⚠️ **「定稿后提交」会让追赶单里的 HEAD 期望值失效**（2026-09-17 踩到）：
> 操作单第 1 步原本写「HEAD 应等于 `e78967e`」；但定稿后为了记录本次工作又提交了 `bc9c7db`，
> 家里 pull 会落到 `bc9c7db` → 照文档核对会**误判为 pull 失败**。
> **纪律：追赶单里的 HEAD / md5 / 条数这类「期望值」，必须在最后一次提交之后才定稿**；
> 若定稿后仍要提交，记得回头改期望值（本次已改，并把该案例写进操作单供家里理解）。
